...
class  KYCSaneWidget: public QWidget
{
    Q_OBJECT
private:
    explicit KYCSaneWidget(QWidget *parent = nullptr);
    ~KYCSaneWidget();
    KYCSaneWidget(const KYCSaneWidget &) = delete ;
    KYCSaneWidget &operator = (const KYCSaneWidget &) = delete ;

public:

    // 线程安全,不用生成对象，防止内存泄漏，是一种很好的懒汉单例模式
    static KYCSaneWidget &getInstance()
    {
        static KYCSaneWidget instance; //局部静态变量
        return instance;
    }
